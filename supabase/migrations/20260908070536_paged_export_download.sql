-- Bound each revalidation request. CSV only needs its primary table; XLSX follows all parts.
create function private.export_page(org uuid,job uuid,part_index integer default 0,row_offset integer default 0) returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 execute format('select coalesce(jsonb_agg(e.value order by e.n),''[]'') from (select value,n from jsonb_array_elements($2) with ordinality as x(value,n) where n>$3 and n<=$3+500) e where exists(select 1 from public.%I r where r.organization_id=$1 and r.%I=(e.value->>%L)::uuid and (%s))',t,k,k,private.export_predicate(t)) into rows using org,part->'rows',row_offset;
...
create function public.export_page(org uuid,job uuid,part_index integer default 0,row_offset integer default 0) returns jsonb language sql security invoker set search_path='' as $$select private.export_page(org,job,part_index,row_offset);$$;
revoke all on function private.export_page(uuid,uuid,integer,integer),public.export_page(uuid,uuid,integer,integer) from public,anon,authenticated;
grant execute on function private.export_page(uuid,uuid,integer,integer),public.export_page(uuid,uuid,integer,integer) to authenticated;
