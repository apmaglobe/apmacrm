-- Admins may prepare a marketing plan before the CRM deal exists. It becomes operational only after attachment.
alter table public.marketing_plans alter column deal_id drop not null;
create or replace function private.can_read_marketing_plan(org uuid, plan uuid) returns boolean language sql stable security definer set search_path='' as $$
 select private.member_id(org) is not null and exists(select 1 from public.marketing_plans p where p.organization_id=org and p.id=plan and p.status<>'archived' and (p.deal_id is null and private.is_admin(org) or p.deal_id is not null and private.can_read_deal(org,p.deal_id)));
$$;
create or replace function private.can_manage_marketing_plan(org uuid, plan uuid) returns boolean language sql stable security definer set search_path='' as $$
 select private.member_id(org) is not null and exists(select 1 from public.marketing_plans p where p.organization_id=org and p.id=plan and (private.is_admin(org) or p.deal_id is not null and private.permitted(org,'marketing.write') and (p.created_by=private.member_id(org) or private.can_edit_deal(org,p.deal_id))));
$$;
...
begin
 ...
 if operation='plan.save' then
   ...
   if plan.id is null then
     ...
     insert into public.marketing_plans(id,organization_id,deal_id,title,created_by) values(rid,org,nullif(payload->>'deal_id','')::uuid,payload->>'title',m);
   else
     ...
     if plan.deal_id is null and nullif(payload->>'deal_id','') is not null then
       ...
       select * into deal from public.deals where organization_id=org and id=(payload->>'deal_id')::uuid for update;
       if deal.id is null or deal.first_confirmed_at is null then raise exception 'MARKETING_PLAN_DENIED' using errcode='42501'; end if;
       update public.marketing_plans set deal_id=deal.id,title=coalesce(nullif(trim(payload->>'title'),''),title),version=version+1,updated_at=now() where organization_id=org and id=rid;
     else
