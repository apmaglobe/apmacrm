create index customers_import_match_name on public.customers(organization_id,lower(btrim(name))) where not archived;
...
declare job public.import_jobs;r jsonb;c public.customers;cid uuid;ext text;idx int;lim int;result jsonb;prior private.requests;change record;loc_before jsonb;con_before jsonb;begin
 ...
 return (select jsonb_build_object('candidates',coalesce(jsonb_agg(jsonb_build_object('index',n-1,'matches',(select coalesce(jsonb_agg(to_jsonb(candidate)),'[]') from (select candidate_row.id,candidate_row.name,candidate_row.external_id from public.customers candidate_row where candidate_row.organization_id=org and not candidate_row.archived and (lower(trim(candidate_row.name))=lower(trim(rw->>'customer_name')) or candidate_row.external_id=nullif(rw->>'external_id','')) order by (candidate_row.external_id=nullif(rw->>'external_id','')) desc nulls last,candidate_row.id limit 20) candidate))),'[]')) from jsonb_array_elements(payload->'rows') with ordinality x(rw,n));
