...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await db.rpc("download_export", {org, job});
  if (error) {
    console.warn("export.download", {code:error.code});
    return NextResponse.json({error:"İxrac hazır deyil, vaxtı bitib və ya icazəniz dəyişib."},{status:error.code==="42501"?403:error.code==="57014"?503:409});
  }
  const {module:section,format,tables:rawResults} = data as {module:string;format:string;tables:{table:string;rows:Record<string,unknown>[]}[]};
