...
export function Tasks({ org, data, member, refresh, q }: PanelProps) {
  ...
  const visible = status === "all" ? tasks : tasks.filter((item) => item.status === status);
...
function TaskForm({ task, members, assigned, onClose, onSave }: { task: Item | null; members: Item[]; assigned: string[]; onClose: () => void; onSave: (form: FormData) => Promise<void> }) {
  return <Modal title={task ? "Taskı düzəlt" : "Daxili task əlavə et"} onClose={onClose}><Form onSave={onSave} label="Saxla"><Field name="title" label="Taskın adı" value={task?.title} required/><label>İzah və görüləcək işlər<textarea name="description" defaultValue={task?.description ?? ""} rows={5} placeholder="Məsələn: müştərilərə zəng edin, şəxsi səhifənin paylaşım planını yoxlayın…"/></label><Field name="due_at" label="Deadline" type="datetime-local" value={localInput(task?.due_at)}/><label>Cavabdehlər<select name="assignees" multiple required defaultValue={assigned} size={Math.min(Math.max(members.length, 4), 8)}>{members.filter((member) => member.status === "active").map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select><small>Bir neçə nəfəri Command/Control ilə seçə bilərsiniz.</small></label></Form></Modal>;
}
